...
**2. 성능 검증 (CodeT5+)**
  ...
  1. 이상치 점수, 일관성 점수, CVSS 점수는 0과 1 사이의 값으로 정규화 한 다음, 모든 점수를 합산한 점수(R-score)를 기준으로 데이터를 내림차순 정렬
  2. 최적의 필터링 비율을 탐색하기 위해 10%, 20%, 30%, 40%, 50% 각각의 비율로 데이터를 제거했을 때, 그 데이터를 학습한 CodeT5+ 모델이 생성한 코드를 확인
